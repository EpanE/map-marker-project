...
def create_map():
    ...
    for index, row in df.iterrows():
        # Define the details to show in the popup and tooltip
        details = f"<b>Name:</b> {row['Label']}<br><b>Latitude:</b> {row['Latitude']}<br><b>Longitude:</b> {row['Longitude']}<br><b>Color:</b> {row['Color']}<br><b>Icon:</b> {row['Icon']}"
        tooltip_text = f"{row['Label']} ({row['Latitude']}, {row['Longitude']})"
...
icon_var.set('info-sign')  # default icon
icon_menu = ttk.OptionMenu(main_frame, icon_var, 'info-sign', 'info-sign', 'cloud', 'home', 'star')
icon_menu.grid(row=4, column=1, padx=5, pady=5, sticky=(tk.W, tk.E))

# Zoom and map style options
ttk.Label(main_frame, text="Zoom Level:").grid(row=5, column=0, padx=5, pady=5, sticky=tk.W)
zoom_scale = tk.Scale(main_frame, from_=1, to=18, orient=tk.HORIZONTAL)
zoom_scale.set(12)  # default zoom level
zoom_scale.grid(row=5, column=1, padx=5, pady=5, sticky=(tk.W, tk.E))

ttk.Label(main_frame, text="Map Style:").grid(row=6, column=0, padx=5, pady=5, sticky=tk.W)
map_style = tk.StringVar()
map_style.set('OpenStreetMap')  # default map style
map_style_menu = ttk.OptionMenu(main_frame, map_style, 'OpenStreetMap', 'Stamen Terrain', 'Stamen Toner', 'Stamen Watercolor')
map_style_menu.grid(row=6, column=1, padx=5, pady=5, sticky=(tk.W, tk.E))

# Buttons for adding, editing, and deleting markers
add_marker_btn = ttk.Button(main_frame, text="Add/Update Marker", command=add_marker)
add_marker_btn.grid(row=7, column=1, padx=5, pady=5, sticky=tk.E)

delete_marker_btn = ttk.Button(main_frame, text="Delete Marker", command=delete_marker)
delete_marker_btn.grid(row=7, column=2, padx=5, pady=5, sticky=tk.W)

# Listbox to display markers
coordinates_listbox = tk.Listbox(main_frame, height=10)
coordinates_listbox.grid(row=9, column=0, columnspan=3, padx=5, pady=5, sticky=(tk.W, tk.E, tk.N, tk.S))
coordinates_listbox.bind('<<ListboxSelect>>', edit_marker)

# Search box
ttk.Label(main_frame, text="Search:").grid(row=10, column=0, padx=5, pady=5, sticky=tk.W)
search_entry = ttk.Entry(main_frame)
search_entry.grid(row=10, column=1, padx=5, pady=5, sticky=(tk.W, tk.E))

search_btn = ttk.Button(main_frame, text="Search", command=search_markers)
search_btn.grid(row=10, column=2, padx=5, pady=5, sticky=tk.W)

# Save and Load buttons
save_btn = ttk.Button(main_frame, text="Save Project", command=save_project)
save_btn.grid(row=11, column=1, padx=5, pady=5, sticky=tk.E)

load_btn = ttk.Button(main_frame, text="Load Project", command=load_project)
load_btn.grid(row=11, column=2, padx=5, pady=5, sticky=tk.W)

# Buttons to create the map and export as image
create_map_btn = ttk.Button(main_frame, text="Create Map", command=create_map)
create_map_btn.grid(row=12, column=1, padx=5, pady=5, sticky=tk.E)

export_map_btn = ttk.Button(main_frame, text="Export Map as Image", command=export_map_as_image)
export_map_btn.grid(row=12, column=2, padx=5, pady=5, sticky=tk.W)
